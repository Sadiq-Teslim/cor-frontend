...
function parseVoice(
  text: string,
  step: number,
): { field: keyof OnboardingFormData; value: string } | null {
  ...
  switch (step) {
    ...
    case 3: {
      if (/\b(male|man|boy|guy)\b/i.test(t))
        return { field: "sex", value: "Male" };
      if (/\b(female|woman|girl|lady)\b/i.test(t))
        return { field: "sex", value: "Female" };
      if (/\b(prefer|rather|skip)\b/i.test(t))
        return { field: "sex", value: "Prefer not to say" };
      return null;
    }
    case 4: {
      if (/\b(not sure|don't know|unsure|no idea|maybe)\b/i.test(t))
        return { field: "highBP", value: "Not sure" };
      if (/\b(yes|yeah|yep|i have|diagnosed)\b/i.test(t))
        return { field: "highBP", value: "Yes" };
      if (/\b(no|nah|nope|never|haven't)\b/i.test(t))
        return { field: "highBP", value: "No" };
      return null;
    }
    case 5: {
      if (/\b(yes|yeah|yep|i do|i take)\b/i.test(t))
        return { field: "medication", value: "Yes" };
      if (/\b(no|nah|nope|i don't|don't take)\b/i.test(t))
        return { field: "medication", value: "No" };
      return null;
    }
    case 6: {
      if (/\b(sometimes|occasionally|once in a while|a little)\b/i.test(t))
        return { field: "smokeDrink", value: "Sometimes" };
      if (/\b(yes|yeah|yep|i do|i smoke|i drink)\b/i.test(t))
        return { field: "smokeDrink", value: "Yes" };
      if (/\b(no|nah|nope|i don't|never)\b/i.test(t))
        return { field: "smokeDrink", value: "No" };
      return null;
    }
    case 7: {
      if (/\b(active|very active|exercise|workout|gym)\b/i.test(t))
        return { field: "activity", value: "Active" };
      if (/\b(moderate|somewhat|fairly|a bit)\b/i.test(t))
        return { field: "activity", value: "Moderate" };
      if (/\b(sedentary|not active|inactive|lazy|sit|desk)\b/i.test(t))
        return { field: "activity", value: "Sedentary" };
      return null;
    }
    case 8: {
      const m = t.match(/\d+/);
...
export default function OnboardingScreen({
  ...
  }, [step, initMic, playQuestion, stopAll]);
  ...
  const voiceIcon = (state: VoiceState) => {
    ...
    if (state === "playing")
      return <Volume2 size={28} style={{ color: "#00E5CC" }} />;
    return (
      <Mic
        size={28}
        style={{ color: state === "recording" ? "#FF6B6B" : "#8896A8" }}
      />
    );
  };
  ...
  const voiceLabel = (state: VoiceState) => {
    switch (state) {
      ...
      case "transcribing":
        return "Processing...";
      default:
        return voiceReady ? "Tap to speak" : "";
    }
  };
  ...
  const circleBorder =
    voiceState === "recording"
      ? "#FF6B6B"
      : voiceState === "playing" || voiceState === "listening"
        ? "#00E5CC"
        : "#1E2D45";
  const circleBg =
    voiceState === "recording"
    ...
    <div className="min-h-screen">
      ...
      <div className="px-8 pb-8">
        ...
        >
          {/* Question */}
          {step === 1 && (
            <h2 className="text-xl font-semibold mb-2">What is your name?</h2>
          )}
          {step === 2 && (
            <h2 className="text-xl font-semibold mb-2">How old are you?</h2>
          )}
          {step === 3 && (
            <h2 className="text-xl font-semibold mb-2">Biological sex?</h2>
          )}
          {step === 4 && (
          ...
          {step === 7 && (
            <h2 className="text-xl font-semibold mb-2">How active are you?</h2>
          )}
          {step === 8 && (
            <h2 className="text-xl font-semibold mb-2">
          ...
          <div className="flex flex-col items-center my-5">
            ...
            <div className="text-xs mt-2" style={{ color: "#8896A8" }}>
            ...
            {voiceTranscript && (
              <div
                ...
                style={{ color: "#00E5CC", background: "rgba(0,229,204,0.08)" }}
              >
          ...
          {step === 1 && (
            <input
              ...
              value={data.name}
              onChange={(e) =>
                onUpdateData({ ...dataRef.current, name: e.target.value })
              }
          ...
          {step === 2 && (
            <input
              ...
              value={data.age}
              onChange={(e) =>
                onUpdateData({ ...dataRef.current, age: e.target.value })
              }
              className="w-full px-4 py-3 text-base rounded-xl"
              style={{
                background: "#0A0F1E",
          ...
          {step === 3 &&
            renderChips(["Male", "Female", "Prefer not to say"], "sex")}
          ...
          {step === 7 &&
            renderChips(["Sedentary", "Moderate", "Active"], "activity")}
          {step === 8 && (
            <input
              type="number"
              min="1"
              ...
              value={data.sleep}
              onChange={(e) =>
                onUpdateData({ ...dataRef.current, sleep: e.target.value })
              }
              className="w-full px-4 py-3 text-base rounded-xl"
              style={{
                background: "#0A0F1E",
        ...
        <button
          ...
          style={{
            background: isSubmitting ? "#888" : "#00E5CC",
            color: "#0A0F1E",
          }}
        >
